...
func udfBindTyped[T tableSource](info C.duckdb_bind_info) {
	...
	for i, v := range columns {
		if v.T == nil {
			setBindError(info, errMalformedType.Error())
		}
		dt := v.T.logicalType()
...
func getValue(t TypeInfo, v C.duckdb_value) (any, error) {
	...
	case TYPE_TIME:
		...
		return time.UnixMicro(int64(val.micros)).UTC(), nil
	case TYPE_INTERVAL:
		intr := C.duckdb_get_interval(v)
		return Interval{
			Days:   int32(intr.days),
			Months: int32(intr.months),
			Micros: int64(intr.micros),
		}, nil
	case TYPE_HUGEINT:
