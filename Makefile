...
deps.darwin.amd64:
	...
	g++ -std=c++11 --target=x86_64-apple-macos11 -DGODUCKDB_FROM_SOURCE -DNDEBUG -c duckdb.cpp
...
deps.darwin.arm64:
	if [ "$(shell uname -s | tr '[:upper:]' '[:lower:]')" != "darwin" ]; then echo "Error: must run build on darwin"; false; fi
	g++ -std=c++11 --target=arm64-apple-macos11 -DGODUCKDB_FROM_SOURCE -DNDEBUG -c duckdb.cpp
	ar rvs libduckdb.a duckdb.o
...
deps.linux.amd64:
	...
	g++ -std=c++11 -DGODUCKDB_FROM_SOURCE -DNDEBUG -c duckdb.cpp
	ar rvs libduckdb.a duckdb.o
...
deps.linux.arm64:
	if [ "$(shell uname -s | tr '[:upper:]' '[:lower:]')" != "linux" ]; then echo "Error: must run build on linux"; false; fi
	aarch64-linux-gnu-g++ -std=c++11 -DGODUCKDB_FROM_SOURCE -DNDEBUG -c duckdb.cpp
